...
PLOT_CONFIG <- list(
  plot_A = list(
    ...
    annotation_configs = list(
      ...
      list(suffix = "7", label = "Reference Label (for referencing figures):", placeholder = "Must start with 'fig-' , ie fig-A-1, etc")
    )
  ),
  plot_B = list(
  ...
  for (i in 1:n_plots) {
    ...
    if (config$has_annotations) {
      for (j in 1:config$num_annotations) {
        ...
        # For annotation 7 (reference label), provide default if empty
        if (j == 7) {
          ...
          if (!is.null(x_var)) {
            ...
            if (config$has_annotations) {
              for (j in 1:config$num_annotations) {
                ...
                Annotation_Labels <- c("Title", "Subtitle", "Table 1", "Plot 1", "Plot 2", "Overall Figure", "Reference Label")
...
get_plot_values <- function(plot_type, i, input) {
  ...
  if (plot_type == "plot_A") {
    ...
    table_caption_input <- input[[paste0(plot_type, "_annotation_3_", i)]]
    plot_1_caption_input <- input[[paste0(plot_type, "_annotation_4_", i)]]
    plot_2_caption_input <- input[[paste0(plot_type, "_annotation_5_", i)]]
    figure_overall_caption_input <- input[[paste0(plot_type, "_annotation_6_", i)]]
    figure_reference_input <- input[[paste0(plot_type, "_annotation_7_", i)]]
    ...
    tblCap <- if (!is.null(table_caption_input) && table_caption_input != "") {
      table_caption_input
    } else {
    ...
    plot2Cap <- paste0("Panel ", Roman_numerals[i], ".3") 
    figOvCap <- paste0("Figure ", " (", Roman_numerals[i], ")") 
    figRef <- paste0("@fig-", substr(plot_type, nchar(plot_type), nchar(plot_type)), "-", Roman_numerals[i]) # ie fig-A-I,fig-A-II, ...
  }
