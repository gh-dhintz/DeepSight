  ...
  if (!file.exists(REPORT_LOG_FILE)) {
    ...
    for (i in 1:MAX_PLOTS_A) {
      header_cols <- c(header_cols,
                      ...
                      paste0("plot_A_annotation_title_", i),
                      paste0("plot_A_annotation_subtitle_", i),
                      paste0("plot_A_annotation_table_caption_", i),
                      paste0("plot_A_annotation_plot1_caption_", i),
                      paste0("plot_A_annotation_plot2_caption_", i),
                      paste0("plot_A_annotation_overall_caption_", i))
    }
...
log_report_generation <- function(all_inputs, format, bookmark_url) {
  ...
  for (i in 1:MAX_PLOTS_A) {
    ...
    annotation_title_name <- paste0("plot_A_annotation_title_", i)
    annotation_subtitle_name <- paste0("plot_A_annotation_subtitle_", i)
    annotation_table_caption_name <- paste0("plot_A_annotation_table_caption_", i)
    annotation_plot1_caption_name <- paste0("plot_A_annotation_plot1_caption_", i)
    annotation_plot2_caption_name <- paste0("plot_A_annotation_plot2_caption_", i)
    annotation_overall_caption_name <- paste0("plot_A_annotation_overall_caption_", i)
    ...
    log_entry[[annotation_title_name]] <- ifelse(is.null(all_inputs[[annotation_title_name]]), "", all_inputs[[annotation_title_name]])
    log_entry[[annotation_subtitle_name]] <- ifelse(is.null(all_inputs[[annotation_subtitle_name]]), "", all_inputs[[annotation_subtitle_name]])
    log_entry[[annotation_table_caption_name]] <- ifelse(is.null(all_inputs[[annotation_table_caption_name]]), "", all_inputs[[annotation_table_caption_name]])
    log_entry[[annotation_plot1_caption_name]] <- ifelse(is.null(all_inputs[[annotation_plot1_caption_name]]), "", all_inputs[[annotation_plot1_caption_name]])
    log_entry[[annotation_plot2_caption_name]] <- ifelse(is.null(all_inputs[[annotation_plot2_caption_name]]), "", all_inputs[[annotation_plot2_caption_name]])
    log_entry[[annotation_overall_caption_name]] <- ifelse(is.null(all_inputs[[annotation_overall_caption_name]]), "", all_inputs[[annotation_overall_caption_name]])
  }
...
output$plot_A_inputs_ui <- renderUI({
  ...
  style_tag <- tags$style(HTML("
    /*
    Using styles.css for now
    input[id*='plot_A_annotation_']::placeholder {
  ...
  plot_inputs <- lapply(1:plot_count_trigger, function(i) {
    ...
    current_annotation_title <- isolate(input[[annotation_title_name]])
    current_annotation_subtitle <- isolate(input[[annotation_subtitle_name]])
    current_annotation_table_caption <- isolate(input[[annotation_table_caption_name]])
    current_annotation_plot1_caption <- isolate(input[[annotation_plot1_caption_name]])
    current_annotation_plot2_caption <- isolate(input[[annotation_plot2_caption_name]])
    current_annotation_overall_caption <- isolate(input[[annotation_overall_caption_name]])

    # GET CURRENT VALUES OR USE DEFAULTS
    current_x <- isolate(input[[x_var_name]])  # Use isolate() to prevent reactivity
    current_subcaption <- isolate(input[[subcaption_name]])  # Use isolate()ggplo
    ...
    div(
      ...
      h6(
        a(
          href = paste0("#collapse_plot_A_", i),
          `data-toggle` = "collapse",
          `aria-expanded` = "false",
          `aria-controls` = paste0("collapse_plot_A_", i),
          style = "text-decoration: none; color: inherit;",
          tags$i(class = "fa fa-caret-right"),
          paste0(" Plot Annotations for ", PLOT_A_LABEL, " (", i, ")")
        ),
        class = "text-secondary", style = "font-weight: bold; cursor: pointer;"
      ),
      div(
        class = "collapse",
        id = paste0("collapse_plot_A_", i),
        textInput(annotation_title_name,
              div("Annotation: Title", style = "font-size: 12px; font-weight: bold;"),
              value = if(!is.null(current_annotation_title)) current_annotation_title else "",
              placeholder = "Override plot title"),
        textInput(annotation_subtitle_name,
              div("Annotation: Subtitle", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_annotation_subtitle)) current_annotation_subtitle else "",
              placeholder = "Override plot subtitle"),
        textInput(annotation_table_caption_name,
              div("Annotation: Table Caption", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_annotation_table_caption)) current_annotation_table_caption else "",
              placeholder = "Override table caption"),
        textInput(annotation_plot1_caption_name,
              div("Annotation: Plot 1 Caption", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_annotation_plot1_caption)) current_annotation_plot1_caption else "",
              placeholder = "Override plot 1 caption"),
        textInput(annotation_plot2_caption_name,
              div("Annotation: Plot 2 Caption", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_annotation_plot2_caption)) current_annotation_plot2_caption else "",
              placeholder = "Override plot 2 caption"),
        textInput(annotation_overall_caption_name,
              div("Annotation: Overall Caption", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_annotation_overall_caption)) current_annotation_overall_caption else "",
              placeholder = "Override overall plot caption")
      )
    )
  })
...
text_and_dropdown_inputs <- reactive({
  ...
  if (!is.null(input$plot_A_n) && input$plot_A_n > 0) {
    for (i in 1:input$plot_A_n) {
      ...
      all_vals[[annotation_title_name]] <- input[[annotation_title_name]]
      all_vals[[annotation_subtitle_name]] <- input[[annotation_subtitle_name]]
      all_vals[[annotation_table_caption_name]] <- input[[annotation_table_caption_name]]
      all_vals[[annotation_plot1_caption_name]] <- input[[annotation_plot1_caption_name]]
      all_vals[[annotation_plot2_caption_name]] <- input[[annotation_plot2_caption_name]]
      all_vals[[annotation_overall_caption_name]] <- input[[annotation_overall_caption_name]]
    }
  }
...
observeEvent(text_debounced(), {
  ...
  loadPresetReport <- function(preset_url) {
    ...
    if (length(extracted) > 0) {
      ...
      observe({
        ...
        update_dynamic_inputs("plot_A_annotation_title_", "text")
        update_dynamic_inputs("plot_A_annotation_subtitle_", "text")
        update_dynamic_inputs("plot_A_annotation_table_caption_", "text")
        update_dynamic_inputs("plot_A_annotation_plot1_caption_", "text")
        update_dynamic_inputs("plot_A_annotation_plot2_caption_", "text")
        update_dynamic_inputs("plot_A_annotation_overall_caption_", "text")
  ...
  observeEvent(input$load_basic_report_two, {
    preset_url <- "http://127.0.0.1:4001/?_inputs_&nav=%22Report%20Export%22&plots_subtabs=%22My%20Second%20Plot%22&load_basic_report=0&load_basic_report_two=0&update_and_copy_url=0&download_trigger=1&parse_url_1=0&parse_url_2=0&load_report_1=0&load_report_2=0&clear_comparison=0&refresh_log=0&clear_log=0&format=%22PDF%22&plot_A_n=3&plot_B_n=3&bookmark_url_1=%22%22&bookmark_url_2=%22%22&title=%22My%20Report%20Title%22&subtitle=%22Analysis%20Report%22&name=%22John%20Doe%22&reportLogTable_rows_selected=null&reportLogTable_columns_selected=null&reportLogTable_cells_selected=%5B%5D&reportLogTable_rows_current=1&reportLogTable_rows_all=1&reportLogTable_state=%7B%22time%22%3A1750956379937%2C%22start%22%3A0%2C%22length%22%3A10%2C%22order%22%3A%5B%5D%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22smart%22%3Atrue%2C%22regex%22%3Afalse%2C%22caseInsensitive%22%3Atrue%7D%2C%22columns%22%3A%5B%7B%22visible%22%3Atrue%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22smart%22%3Atrue%2C%22regex%22%3Afalse%2C%22caseInsensitive%22%3Atrue%7D%7D%2C%7B%22visible%22%3Atrue%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22smart%22%3Atrue%2C%22regex%22%3Afalse%2C%22caseInsensitive%22%3Atrue%7D%7D%2C%7B%22visible%22%3Atrue%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22smart%22%3Atrue%2C%22regex%22%3Afalse%2C%22caseInsensitive%22%3Atrue%7D%7D%2C%7B%22visible%22%3Atrue%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22smart%22%3Atrue%2C%22regex%22%3Afalse%2C%22caseInsensitive%22%3Atrue%7D%7D%2C%7B%22visible%22%3Atrue%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22smart%22%3Atrue%2C%22regex%22%3Afalse%2C%22caseInsensitive%22%3Atrue%7D%7D%2C%7B%22visible%22%3Atrue%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22smart%22%3Atrue%2C%22regex%22%3Afalse%2C%22caseInsensitive%22%3Atrue%7D%7D%2C%7B%22visible%22%3Atrue%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22smart%22%3Atrue%2C%22regex%22%3Afalse%2C%22caseInsensitive%22%3Atrue%7D%7D%2C%7B%22visible%22%3Atrue%2C%22search%22%3A%7B%22search%22%3A%22%22%2C%22s... [truncated]"
...
observeEvent(input$update_and_copy_url, {
  ...
  if (copy_after_bookmark()) {
    ...
    if (download_requested()) {
      ...
      plot_patterns <- c("^plot_A_x", "^plot_B_x", "^plot_A_SubCaption",
                   "^plot_A_annotation_title_", "^plot_A_annotation_subtitle_",
                   "^plot_A_annotation_table_caption_", "^plot_A_annotation_plot1_caption_",
                   "^plot_A_annotation_plot2_caption_", "^plot_A_annotation_overall_caption_",
                   "^plot_B_SubCaption")
      ...
      for (plot_type in c("plot_A", "plot_B")) {
        ...
        if (!is.null(n_plots) && n_plots > 0) {
          ...
          annotation_title_name <- paste0(plot_type, "_annotation_title_", i)
          annotation_subtitle_name <- paste0(plot_type, "_annotation_subtitle_", i)
          annotation_table_caption_name <- paste0(plot_type, "_annotation_table_caption_", i)
          annotation_plot1_caption_name <- paste0(plot_type, "_annotation_plot1_caption_", i)
          annotation_plot2_caption_name <- paste0(plot_type, "_annotation_plot2_caption_", i)
          annotation_overall_caption_name <- paste0(plot_type, "_annotation_overall_caption_", i)

          if (!is.null(input[[annotation_title_name]])) {
            all_inputs[[annotation_title_name]] <- input[[annotation_title_name]]
          }
          if (!is.null(input[[annotation_subtitle_name]])) {
            all_inputs[[annotation_subtitle_name]] <- input[[annotation_subtitle_name]]
          }
          if (!is.null(input[[annotation_table_caption_name]])) {
            all_inputs[[annotation_table_caption_name]] <- input[[annotation_table_caption_name]]
          }
          if (!is.null(input[[annotation_plot1_caption_name]])) {
            all_inputs[[annotation_plot1_caption_name]] <- input[[annotation_plot1_caption_name]]
          }
          if (!is.null(input[[annotation_plot2_caption_name]])) {
            all_inputs[[annotation_plot2_caption_name]] <- input[[annotation_plot2_caption_name]]
          }
          if (!is.null(input[[annotation_overall_caption_name]])) {
            all_inputs[[annotation_overall_caption_name]] <- input[[annotation_overall_caption_name]]
          }
        }
      ...
      isolate({
        fmt <- switch(input$format,
          PDF = list(input = "pdf_8.qmd", format = "pdf", ext = "pdf"),
...
for (i in 1:MAX_PLOTS_A) {
  local({
    ...
    output[[paste0("Plot_A_Obj", my_i)]] <- renderPlot({
      ...
      title_trigger <- input[[paste0("plot_A_annotation_title_", my_i)]]
      subtitle_trigger <- input[[paste0("plot_A_annotation_subtitle_", my_i)]]
      table_caption_trigger <- input[[paste0("plot_A_annotation_table_caption_", my_i)]]
      plot1_caption_trigger <- input[[paste0("plot_A_annotation_plot1_caption_", my_i)]]
      plot2_caption_trigger <- input[[paste0("plot_A_annotation_plot2_caption_", my_i)]]
      overall_caption_trigger <- input[[paste0("plot_A_annotation_overall_caption_", my_i)]]

      # Set plot titles and captions, using annotation if available, otherwise default
      my_title <- if(!is.null(title_trigger) && title_trigger != "") title_trigger else paste0(PLOT_A_LABEL, " (", my_i, ")")
      my_subtitle <- if(!is.null(subtitle_trigger) && subtitle_trigger != "") subtitle_trigger else ""
      my_table_caption <- if(!is.null(table_caption_trigger) && table_caption_trigger != "") table_caption_trigger else "Table 1"
      my_plot1_caption <- if(!is.null(plot1_caption_trigger) && plot1_caption_trigger != "") plot1_caption_trigger else "Plot 1"
      my_plot2_caption <- if(!is.null(plot2_caption_trigger) && plot2_caption_trigger != "") plot2_caption_trigger else "Plot 2"
      my_overall_caption <- if(!is.null(overall_caption_trigger) && overall_caption_trigger != "") overall_caption_trigger else ""
  ...
  theme_border <- theme_void() +
    theme(
      ...
      plot.title = element_text(
        ...
        face = "bold",
        family = "Helvetica Neue",
        margin = margin(t = 15, b = 10, unit = "pt")  # Add bottom margin to title
      ),
      plot.subtitle = element_text(
        size = 16,
        hjust = 0.5,
        family = "Helvetica Neue",
        margin = margin(t = 0, b = 85, unit = "pt")  # Add bottom margin to title
      ),
      plot.caption = element_text(
        size = 17,
        colour = "#C0C0C0",
      ...
      if (section_visited && !is.null(input[[paste0(plot_type, "_n")]]) && input[[paste0(plot_type, "_n")]] > 0) {
        ...
        for (i in 1:n_plots) {
          ...
          if (!is.null(x_var)) {
            ...
            # Add all annotation captions
            annotation_title_var <- input[[paste0(plot_type, "_annotation_title_", i)]]
            annotation_subtitle_var <- input[[paste0(plot_type, "_annotation_subtitle_", i)]]
            annotation_table_caption_var <- input[[paste0(plot_type, "_annotation_table_caption_", i)]]
            annotation_plot1_caption_var <- input[[paste0(plot_type, "_annotation_plot1_caption_", i)]]
            annotation_plot2_caption_var <- input[[paste0(plot_type, "_annotation_plot2_caption_", i)]]
            annotation_overall_caption_var <- input[[paste0(plot_type, "_annotation_overall_caption_", i)]]

            if (!is.null(annotation_title_var) && annotation_title_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Annotation: Title"),
                Value = if(nchar(annotation_title_var) > 50) paste0(substr(annotation_title_var, 1, 50), "...") else annotation_title_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(annotation_subtitle_var) && annotation_subtitle_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Annotation: Subtitle"),
                Value = if(nchar(annotation_subtitle_var) > 50) paste0(substr(annotation_subtitle_var, 1, 50), "...") else annotation_subtitle_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(annotation_table_caption_var) && annotation_table_caption_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Annotation: Table Caption"),
                Value = if(nchar(annotation_table_caption_var) > 50) paste0(substr(annotation_table_caption_var, 1, 50), "...") else annotation_table_caption_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(annotation_plot1_caption_var) && annotation_plot1_caption_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Annotation: Plot 1 Caption"),
                Value = if(nchar(annotation_plot1_caption_var) > 50) paste0(substr(annotation_plot1_caption_var, 1, 50), "...") else annotation_plot1_caption_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(annotation_plot2_caption_var) && annotation_plot2_caption_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Annotation: Plot 2 Caption"),
                Value = if(nchar(annotation_plot2_caption_var) > 50) paste0(substr(annotation_plot2_caption_var, 1, 50), "...") else annotation_plot2_caption_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(annotation_overall_caption_var) && annotation_overall_caption_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Annotation: Overall Caption"),
                Value = if(nchar(annotation_overall_caption_var) > 50) paste0(substr(annotation_overall_caption_var, 1, 50), "...") else annotation_overall_caption_var,
                stringsAsFactors = FALSE
              ))
            }
          }
