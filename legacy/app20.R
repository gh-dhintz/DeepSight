...
server <- function(input, output, session) {
  ...
  # ─────────────────────────────────────────────────
  #### Async UI Generation Processing ####
  # ─────────────────────────────────────────────────
  
  # Poll for completed UI generation tasks
  observe({
    invalidateLater(100, session)  # Check every 100ms for faster UI updates
    
    # Check for completed Plot A UI tasks
    if (controller$nonempty()) {
      result <- controller$pop()
      if (!is.null(result) && !is.null(result$result)) {
        task_name <- result$name
        
        if (grepl("^plotA_ui_", task_name)) {
          # Plot A UI generation completed
          ui_data <- result$result
          
          # Generate the actual UI elements
          plot_inputs <- lapply(1:ui_data$plot_count, function(i) {
            div(
              if (i > 1) br(),
              selectInput(paste0("plotA_x_", i), paste("X Variable for", PLOT_A_LABEL, i, ":"), 
                         choices = names(mtcars), 
                         selected = ui_data$elements[[i]]$x_var_default),
          ...
          output$plotA_inputs_ui <- renderUI({
            div(plot_inputs, shared_inputs)
          })
          
          ui_generation_in_progress(FALSE)
        }
        
        if (grepl("^plotB_ui_", task_name)) {
          # Plot B UI generation completed
          ui_data <- result$result
          ...
          plot_inputs <- lapply(1:ui_data$plot_count, function(i) {
            div(
              if (i > 1) br(),
              selectInput(paste0("plotB_x_", i), paste("X Variable for", PLOT_B_LABEL, i, ":"), 
                         choices = names(mtcars), 
                         selected = ui_data$elements[[i]]$x_var_default),
              h6(paste("Sub-Caption for", PLOT_B_LABEL, i), class = "text-secondary", style = "font-weight: bold;"),
              p("Sub-Captions are used only in the presence of multiple plots per figure", 
                class = "text-muted", style = "font-size: 12px; margin-bottom: 5px;"),
          ...
          shared_inputs <- div(
            ...
            textInput("plotB_caption_shared", paste("Shared Caption for All", PLOT_B_LABEL), 
                     placeholder = paste("Overall caption for the", PLOT_B_LABEL, "figure group")),
            textAreaInput("plotB_notes_shared", paste("Notes for", PLOT_B_LABEL, "Plots"), 
  ...
  #### Async Bookmark Processing ####
  # ─────────────────────────────────────────────────
  
  # Poll for completed bookmark tasks
  observe({
    invalidateLater(200, session)  # Check every 200ms
  ...
  # Reactive values for async UI generation
  ui_generation_in_progress <- reactiveVal(FALSE)
  ...
  # Plot A dynamic inputs (async with crew)
  output$plotA_inputs_ui <- renderUI({
    req(input$plotA_n)
    
    # Show loading indicator during UI generation
    if (ui_generation_in_progress()) {
      return(div(
    ...
    # Set UI generation in progress
    ui_generation_in_progress(TRUE)
    
    # Push UI generation task to crew worker
    controller$push(
      command = {
        # Simulate complex UI generation work
        plot_count <- input$plotA_n
        ui_elements <- list()
        
        # Generate UI structure data
        for (i in 1:plot_count) {
          ui_elements[[i]] <- list(
            id = i,
            x_var_default = if (i == 1) "wt" else if (i == 2) "hp" else "disp"
          )
        }
        
        list(plot_count = plot_count, elements = ui_elements)
      },
      name = paste0("plotA_ui_", Sys.time())
    )
    
    # Return loading indicator temporarily
    div(
      style = "text-align: center; padding: 20px;",
      tags$i(class = "fa fa-spinner fa-spin", style = "font-size: 24px; color: #cc4c02;"),
      br(), br(),
  ...
  # Plot B dynamic inputs (with throttling)
  output$plotB_inputs_ui <- renderUI({
    req(input$plotB_n)
    
    # Add small delay to prevent rapid UI regeneration
  ...
  # Auto-bookmark whenever tracked inputs change (async with crew)
  observeEvent(inputs_to_track(), {
    ...
    if (!bookmark_in_progress()) {
      ...
      # Push bookmark task to crew worker
      controller$push(
        command = {
          # Simulate bookmark generation work
          Sys.sleep(0.1)  # Small delay to batch changes
          "bookmark_ready"
        },
