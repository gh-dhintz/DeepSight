...
```{r}
#| label: load-packages-and-config
#| include: false
...
if (file.exists("report_params.yaml")) {
  ...
  config <- yaml_data$params  # Rename to 'config' to avoid confusion with YAML header params
} else {
...
# Set Subcaption  
if (all(sapply(plotA_subcap_params, function(x) x == "")) && config$plotA_n > 1){
...
if(config$plotA_n > 1){
  fig.cap = config$plotA_caption_shared
  layout.ncol = 2
...
if (config$plotA_notes_shared != "") {
  cat("\n**Notes for Plot A :**", config$plotA_notes_shared, "\n\n")
  }
```
...
# Set Subcaption  
if (all(sapply(plotB_subcap_params, function(x) x == "")) && config$plotB_n > 1){
...
if(config$plotB_n > 1){
  fig.cap = config$plotB_caption_shared
  layout.ncol = 2
  fig.subcap = subcaption
} else {
  fig.cap = config$plotB_caption_shared
...
# Add notes if they exist
if (config$plotB_notes_shared != "") {
  cat("\n**Notes for Plot B :**", config$plotB_notes_shared, "\n\n")
  }
```