...
# ────────────────────────────────────────────────────────────────
MAX_PLOTS_A <- 3  # Maximum number of Plot A plots
MAX_PLOTS_B <- 3  # Maximum number of Plot B plots
  ...
  fluidPage(
    ...
    navbarPage(
          ...
          tabPanel(PLOT_B_LABEL,
            sidebarLayout(
              sidebarPanel(width = 3,
                ...
                numericInput("plotB_n", paste0("Number of plots (max ", MAX_PLOTS_B ,"):"), value = 1, min = 1, max = MAX_PLOTS_B),
      ...
      tabPanel("Report Export",
        ...
        actionButton("manual_bookmark", "Update Bookmark URL", class = "btn btn-info btn-sm"),
...
server <- function(input, output, session) {
  ...
  # Plot A dynamic inputs (async with crew - fixed)
  output$plotA_inputs_ui <- renderUI({
    req(input$plotA_n)
    
    # Generate UI with preserved values - KEY CHANGE: Check for existing values
    plot_inputs <- lapply(1:input$plotA_n, function(i) {
      x_var_name <- paste0("plotA_x_", i)
      subcaption_name <- paste0("plotA_SubCaption_", i)
      
      # GET CURRENT VALUES OR USE DEFAULTS - THIS IS THE KEY FIX
      current_x <- input[[x_var_name]]
      current_subcaption <- input[[subcaption_name]]
      default_x <- if (i == 1) "wt" else if (i == 2) "hp" else "disp"
      
      div(
        if (i > 1) br(),
        selectInput(x_var_name, 
                   paste("X Variable for", PLOT_A_LABEL, i, ":"), 
                   choices = names(mtcars), 
                   selected = if(!is.null(current_x)) current_x else default_x), # USE CURRENT OR DEFAULT
        h6(paste("Sub-Caption for", PLOT_A_LABEL, i), 
        ...
        textInput(subcaption_name, NULL, 
                 value = if(!is.null(current_subcaption)) current_subcaption else "", # USE CURRENT OR EMPTY
                 placeholder = "Brief description of this plot")
      )
    })

     # PRESERVE SHARED INPUT VALUES TOO
    current_caption <- input[["plotA_caption_shared"]]
    current_notes <- input[["plotA_notes_shared"]]
    ...
    shared_inputs <- div(
      ...
      textInput("plotA_caption_shared", 
               ...
               value = if(!is.null(current_caption)) current_caption else "", # PRESERVE VALUE
      ...
      textAreaInput("plotA_notes_shared", 
                   ...
                   value = if(!is.null(current_notes)) current_notes else "", # PRESERVE VALUE
  ...
  output$plotA_outputs_ui <- renderUI({
   ...
   # Plot B dynamic inputs (REPLACE THIS ENTIRE SECTION)
  output$plotB_inputs_ui <- renderUI({
    req(input$plotB_n)
    
    # Create a stable key based only on the number of plots
    ui_key <- paste0("plotB_ui_", input$plotB_n)
    ...
    plot_inputs <- lapply(1:input$plotB_n, function(i) {
      ...
      # GET CURRENT VALUES OR USE DEFAULTS - KEY FIX HERE TOO
      current_x <- input[[x_var_name]]
      current_subcaption <- input[[subcaption_name]]
      default_x <- if (i == 1) "wt" else if (i == 2) "hp" else "disp"
      ...
      div(
        ...
        div(
          ...
          selectInput(
            inputId = x_var_name, 
            label = paste("X Variable for", PLOT_B_LABEL, i, ":"), 
            choices = names(mtcars), 
            selected = if(!is.null(current_x)) current_x else default_x # USE CURRENT OR DEFAULT
          )
        ),
        h6(paste("Sub-Caption for", PLOT_B_LABEL, i), 
    ...
    # PRESERVE SHARED INPUT VALUES
    current_caption <- input[["plotB_caption_shared"]]
    current_notes <- input[["plotB_notes_shared"]]
  ...
  # ─────────────────────────────────────────────────
  #### Auto-bookmark on input change (EXCLUDING DROPDOWNS) ####
  # ─────────────────────────────────────────────────
  
  # Create a reactive that tracks ONLY non-dropdown inputs
  inputs_to_track_stable <- reactive({
    all_inputs <- list()
    
    # Static inputs (these are safe to auto-bookmark)
    ...
    # Plot numbers (these are safe)
    all_inputs$plotA_n <- input$plotA_n
    all_inputs$plotB_n <- input$plotB_n
    
    # Shared settings (text inputs, not dropdowns)
    ...
    # ONLY include SubCaption inputs (text inputs), NOT the dropdown variables
    if (!is.null(input$plotA_n) && input$plotA_n > 0) {
      for (i in 1:input$plotA_n) {
        subcaption_name <- paste0("plotA_SubCaption_", i)
        all_inputs[[subcaption_name]] <- input[[subcaption_name]]
        # NOTE: We're NOT including plotA_x_ variables here
      }
    }
    ...
    if (!is.null(input$plotB_n) && input$plotB_n > 0) {
      for (i in 1:input$plotB_n) {
        subcaption_name <- paste0("plotB_SubCaption_", i)
        all_inputs[[subcaption_name]] <- input[[subcaption_name]]
        # NOTE: We're NOT including plotB_x_ variables here
      }
    }
  ...
  # Debounced version with shorter delay since we're excluding problematic inputs
  inputs_stable_debounced <- debounce(inputs_to_track_stable, 500)  # 0.5 second delay
  ...
  # Manual bookmark trigger for dropdown changes (with longer debounce)
  dropdown_inputs <- reactive({
    dropdown_vals <- list()
    
    # Only track dropdown variables
    if (!is.null(input$plotA_n) && input$plotA_n > 0) {
      for (i in 1:input$plotA_n) {
        x_var_name <- paste0("plotA_x_", i)
        dropdown_vals[[x_var_name]] <- input[[x_var_name]]
      }
    }
  ...
  # Much longer debounce for dropdown changes
  dropdown_debounced <- debounce(dropdown_inputs, 3000)  # 3 seconds
  
  # Separate observer for dropdown changes with longer delay
  observeEvent(dropdown_debounced(), {
    if (!bookmark_in_progress()) {
      bookmark_in_progress(TRUE)
      ...
      controller$push(
        ...
        name = paste0("bookmark_dropdown_", Sys.time())
      )
    }
  }, ignoreInit = TRUE)
  
  # Manual bookmark trigger
  observeEvent(input$manual_bookmark, {
    if (!bookmark_in_progress()) {
      bookmark_in_progress(TRUE)
