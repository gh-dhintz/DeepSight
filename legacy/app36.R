...
get_plot_values <- function(plot_type, i, input) {
  ...
  # Get annotation captions for BOTH plot_A AND plot_B
  if (plot_type == "plot_A" || plot_type == "plot_B") {  # ← CHANGE: Both plot types now
    title_input <- input[[paste0(plot_type, "_annotation_1_", i)]]
    subtitle_input <- input[[paste0(plot_type, "_annotation_2_", i)]]
    plot_1_caption_input <- input[[paste0(plot_type, "_annotation_3_", i)]]
    plot_2_caption_input <- input[[paste0(plot_type, "_annotation_4_", i)]]
    plot_3_caption_input <- input[[paste0(plot_type, "_annotation_5_", i)]]
    plot_4_caption_input <- input[[paste0(plot_type, "_annotation_6_", i)]]
    figure_overall_caption_input <- input[[paste0(plot_type, "_annotation_7_", i)]]
    figure_reference_input <- input[[paste0(plot_type, "_annotation_8_", i)]]

    title <- if (!is.null(title_input) && title_input != "") {
      title_input
    } else {
      paste0(gsub("[()]", "", PLOT_CONFIG[[plot_type]]$label), ".", Roman_numerals[i])
    }
    
    subtitle <- if (!is.null(subtitle_input) && subtitle_input != "") {
      subtitle_input
    } else {
      ""
    }

    plot1Cap <- if (!is.null(plot_1_caption_input) && plot_1_caption_input != "") {
      plot_1_caption_input
    } else {
      paste0("Panel ", Roman_numerals[i], ".1") 
    }

    plot2Cap <- if (!is.null(plot_2_caption_input) && plot_2_caption_input != "") {
      plot_2_caption_input
    } else {
      paste0("Panel ", Roman_numerals[i], ".2") 
    }

    plot3Cap <- if (!is.null(plot_3_caption_input) && plot_3_caption_input != "") {
      plot_3_caption_input
    } else {
      paste0("Panel ", Roman_numerals[i], ".3") 
    }

    plot4Cap <- if (!is.null(plot_4_caption_input) && plot_4_caption_input != "") {
      plot_4_caption_input
    } else {
      paste0("Panel ", Roman_numerals[i], ".4") 
    }

    figOvCap <- if (!is.null(figure_overall_caption_input) && figure_overall_caption_input != "") {
      figure_overall_caption_input
    } else {
      paste0("Figure ", " (", Roman_numerals[i], ")") 
    }

    figRef <- if (!is.null(figure_reference_input) && figure_reference_input != "") {
      paste0("@", gsub("[^a-zA-Z0-9]", "", figure_reference_input)) 
    } else {
      paste0("@fig-", substr(plot_type, nchar(plot_type), nchar(plot_type)), "-", Roman_numerals[i])
    }
  } else {
    # Default fallback (though this shouldn't be reached now)
    title <- paste0(gsub("[()]", "", PLOT_CONFIG[[plot_type]]$label), ".", Roman_numerals[i])
    subtitle <- ""
    plot1Cap <- paste0("Panel ", Roman_numerals[i], ".1") 
    plot2Cap <- paste0("Panel ", Roman_numerals[i], ".2") 
    plot3Cap <- paste0("Panel ", Roman_numerals[i], ".3") 
    plot4Cap <- paste0("Panel ", Roman_numerals[i], ".4") 
    figOvCap <- paste0("Figure ", " (", Roman_numerals[i], ")") 
    figRef <- paste0("@fig-", substr(plot_type, nchar(plot_type), nchar(plot_type)), "-", Roman_numerals[i])
  }
  ...
  list(
    ...
    plot4Cap = plot4Cap,
    figOvCap = figOvCap,
    figRef = figRef  # ← ADD: Include reference for both plot types
  )
}
...
server <- function(input, output, session) {
  ...
  loadPresetReport <- function(preset_url) {
    ...
    if (length(extracted) > 0) {
      ...
      # Schedule dynamic input updates
      observe({
        ...
        for (plot_type in names(PLOT_CONFIG)) {
          ...
          x_params <- extracted[grepl(x_pattern, names(extracted))]
          ...
          if (!is.na(extracted[[notes_param]])) {
