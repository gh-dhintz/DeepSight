...
plot_A_subcap_params <- vars[grepl("^plot_A_annotation[0-9]", names(vars))]
...
plot_B_x_params <- vars[grepl("^plot_B_x[0-9]", names(vars))]
plot_B_x_params <- plot_B_x_params[!sapply(plot_B_x_params, is.null)]
plot_B_subcap_params <- vars[grepl("^plot_B_annotation[0-9]", names(vars))]
...
if (all(sapply(plot_A_subcap_params, function(x) x == "")) && vars$plot_A_n > 1){
  ...
  subcaption <- sapply(plot_A_params, function(x) paste0("Var: ", x))
} else {
...
# title_label <- (plot_A_params[grepl("^plot_A_annotation_1_", names(plot_A_params))])[[i]] 
# subtitle_label <- (plot_A_params[grepl("^plot_A_annotation_2_", names(plot_A_params))])[[i]]   
# tab1_label <- (plot_A_params[grepl("^plot_A_annotation_3_", names(plot_A_params))])[[i]]   
# plt1_label <- (plot_A_params[grepl("^plot_A_annotation_4_", names(plot_A_params))])[[i]]   
# plt2_label <- (plot_A_params[grepl("^plot_A_annotation_5_", names(plot_A_params))])[[i]]   
# overall_fig_label <- (plot_A_params[grepl("^plot_A_annotation_6_", names(plot_A_params))])[[i]]   
# ref_label <- (plot_A_params[grepl("^plot_A_annotation_7_", names(plot_A_params))])[[i]]  

get_label <- function(pattern, default = "") {
  matches <- plot_A_params[grepl(pattern, names(plot_A_params))]
  if(length(matches) >= i && !is.na(matches[[i]])) matches[[i]] else default
}

# knitr will throw a parsing error with variables with underscores outside code chunks
title.label <- get_label("^plot_A_annotation_1_", "Default Title")
subtitle.label <- get_label("^plot_A_annotation_2_", "Default Subtitle")
tab1.label <- get_label("^plot_A_annotation_3_", "Tab 1")
plt1.label <- get_label("^plot_A_annotation_4_", "Plot 1")
plt2.label <- get_label("^plot_A_annotation_5_", "Plot 2")
overall.fig.label <- get_label("^plot_A_annotation_6_", "Figure")
ref.label <- get_label("^plot_A_annotation_7_", paste0("Ref-not-assigned-from-dashboard-for-plotA-fig-",plot_rep_num))

print("printing refs")
print(list(title.label, subtitle.label, tab1.label, plt1.label, plt2.label, overall.fig.label , ref.label))

rmd_content <- glue::glue("
\```{r}
#| label: fig-plot-A-<<plot_rep_num>>
#| fig-height: 6
#| fig-width: 7
#| echo: false
...
(table_with_caption + p1) / p2 + 
  plot_annotation(
    ...
    caption = overall.fig.label ,  # Use the correctly constructed caption
    tag_levels = '1',
    tag_suffix = ")",
    theme = theme_border
  ) +
