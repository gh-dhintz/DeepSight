...
#1. Throttled Auto-Bookmarking:
#
#✅ Increased delay from 500ms to 1000ms
#✅ Reduces rapid-fire bookmark generation
#
#2. UI Generation Throttling:
#
#✅ Added 100ms delay to prevent rapid UI regeneration
#✅ Smoother transitions when changing plot counts
...
server <- function(input, output, session) {
  ...
  # Plot A dynamic inputs (with throttling)
  ...
  # Auto-bookmark whenever tracked inputs change (with throttling)
  observeEvent(inputs_to_track(), {
    # Use a longer delay to reduce rapid-fire bookmarking
    invalidateLater(1000, session)  # Increased from 500ms to 1000ms
    session$doBookmark()
