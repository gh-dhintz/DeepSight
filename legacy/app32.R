...
PLOT_CONFIG <- list(
  plot_A = list(
    ...
    alternate_configs = list(
      list(suffix = "1", label = "Title:", placeholder = "My Title"),
      list(suffix = "2", label = "Subtitle:", placeholder = "My Subtitle"),
      list(suffix = "3", label = "Table 1 Caption:", placeholder = "My Table Caption"),
      list(suffix = "4", label = "Plot 1 Caption:", placeholder = "My Plot Caption"),
      list(suffix = "5", label = "Plot 2 Caption:", placeholder = "My Plot Caption"),
      list(suffix = "6", label = "Overall Plot Caption:", placeholder = "My Plot Caption"),
      list(suffix = "7", label = "Reference Label:", placeholder = "My Reference Label")
    )
  ),
  plot_B = list(
    label = "ESR1 Dx Landscape",
    max_plots = 6,
    height = "900px",
...
create_single_plot_input_config <- function(plot_type, i, input_values, config) {
  ...
  base_inputs <- list(
    ...
    selectInput(x_var_name, 
               paste0("X Variable for ", config$label, " (", i, ")", ":"), 
  ...
  # Add alternate inputs if configured
  if (config$has_alternates) {
    alternate_section <- create_alternate_section_config(plot_type, i, input_values, config)
    base_inputs <- c(base_inputs, list(alternate_section))
  }
  
  return(div(base_inputs))
}

# Create alternate section based on configuration
create_alternate_section_config <- function(plot_type, i, input_values, config) {
  div(
    h6(
      a(
        ...
        paste0(" Plot Annotations for ", config$label, " (", i, ")")
      ),
    ...
    div(
      ...
      generate_alternate_inputs_config(plot_type, i, input_values, config$alternate_configs)
    )
  )
}

# Generate alternate inputs based on configuration
generate_alternate_inputs_config <- function(plot_type, i, input_values, alternate_configs) {
  inputs <- list()
  
  for (config in alternate_configs) {
    input_name <- paste0(plot_type, "_SubCaption_alternate_", config$suffix, "_", i)
...
create_shared_inputs_config <- function(plot_type, input_values, config) {
  ...
  div(
    ...
    textAreaInput(notes_name, 
                 paste("Notes for", config$label, "Plots"), 
                 value = current_notes %||% "",
                 placeholder = paste("Add your comments about", config$label, "here. When referencing in text, use @fig-", plot_type, " when nplots=1; otherwise, use @fig-", plot_type, "-1, @fig-", plot_type, "-2, etc."), 
...
generate_plot_columns_config <- function(plot_type) {
  ...
  for (i in 1:config$max_plots) {
    ...
    # Add alternate captions if configured
    if (config$has_alternates) {
      for (j in 1:config$num_alternates) {
        columns <- c(columns, paste0(plot_type, "_SubCaption_alternate_", j, "_", i))
      }
    }
  }
...
# Collect plot inputs for a specific plot type
collect_plot_inputs <- function(input, plot_type, n_plots) {
  ...
  for (i in 1:n_plots) {
    ...
    if (!is.null(input[[subcaption_name]])) {
      inputs[[subcaption_name]] <- input[[subcaption_name]]
    }
    
    # Add alternate captions if this plot type has them
    config <- PLOT_CONFIG[[plot_type]]
    if (config$has_alternates) {
      for (j in 1:config$num_alternates) {
        alt_name <- paste0(plot_type, "_SubCaption_alternate_", j, "_", i)
        if (!is.null(input[[alt_name]])) {
          inputs[[alt_name]] <- input[[alt_name]]
        }
      }
    }
  }
...
create_readable_param_name <- function(param) {
  ...
  for (plot_type in names(PLOT_CONFIG)) {
    ...
    # Alternate caption pattern (only for plot types that have alternates)
    if (config$has_alternates) {
      alt_pattern <- paste0("^", plot_type, "_SubCaption_alternate_(\\d+)_(\\d+)$")
      matches <- regmatches(param, regexec(alt_pattern, param))[[1]]
      if (length(matches) > 2) {
        return(paste0("📊 ", config$label, " ", matches[3], " - Alt Caption ", matches[2]))
      }
    }
  }
    ...
    if (section_visited) {
      ...
      if (!is.null(input[[paste0(plot_type, "_n")]]) && input[[paste0(plot_type, "_n")]] > 0) {
        ...
        for (i in 1:n_plots) {
          ...
          if (!is.null(x_var)) {
            plot_details <- rbind(plot_details, data.frame(
              Setting = paste0(config$label, ".", i, " - X Variable"),
              Value = x_var,
              stringsAsFactors = FALSE
            ))
            ...
            if (config$has_alternates) {
              for (j in 1:config$num_alternates) {
                alt_var <- input[[paste0(plot_type, "_SubCaption_alternate_", j, "_", i)]]
                
                if (!is.null(alt_var) && alt_var != "") {
                  plot_details <- rbind(plot_details, data.frame(
                    Setting = paste0(config$label, ".", i, " - Alt Caption ", j),
                    Value = if(nchar(alt_var) > 50) paste0(substr(alt_var, 1, 50), "...") else alt_var,
                    stringsAsFactors = FALSE
                  ))
                }
              }
        ...
        if (!is.null(shared_notes_var) && shared_notes_var != "") {
          plot_details <- rbind(plot_details, data.frame(
            Setting = paste0(config$label, " - Shared Notes"),
...
generate_plot_A_content <- function(i, config, input) {
  ...
  plots <- create_plot_components(dat, i, values)
  table_component <- create_table_component(dat, i)
  
  # Combine with patchwork
  combine_plot_A_components(plots, table_component, values$final_caption, values$title, values$subtitle)
}
...
generate_plot_B_content <- function(i, config, input) {
  ...
  combine_plot_B_components(plots, values$title)
}
...
get_plot_values <- function(plot_type, i, input) {
  ...
  # Only try to get alternate captions for plot_A
  if (plot_type == "plot_A") {
    title_input <- input[[paste0(plot_type, "_SubCaption_alternate_1_", i)]]
    subtitle_input <- input[[paste0(plot_type, "_SubCaption_alternate_2_", i)]]
    
    title <- if (!is.null(title_input) && title_input != "") {
      title_input
    } else {
      paste0(PLOT_CONFIG[[plot_type]]$label, " (", i, ")")
    }
    ...
    title <- paste0(PLOT_CONFIG[[plot_type]]$label, " (", i, ")")
    subtitle <- ""
  }
...
create_plot_components <- function(dat, i, values) {
...
create_table_component <- function(dat, i) {
  table_data <- dat %>%
    select(mpg, cyl, hp) %>%
    head(5) %>%
    round(2)
...
combine_plot_A_components <- function(plots, table_component, final_caption, title, subtitle) {
...
combine_plot_B_components <- function(plots, title) {
...
create_border_theme_B <- function() {
  theme_gray() + 
    theme(
      plot.background = element_rect(fill = NA, colour = '#f4dbcc', size = 1),
      plot.title = element_text(
        size = 16, hjust = 0.5, face = "bold", family = "Arial"
      ),
      ...
      if (!is.null(n_plots) && n_plots > 0) {
        ...
        for (i in 1:n_plots) {
          ...
          if (config$has_alternates) {
            for (j in 1:config$num_alternates) {
              alt_name <- paste0(plot_type, "_SubCaption_alternate_", j, "_", i)
              all_vals[[alt_name]] <- input[[alt_name]]
            }
          }
        }
    ...
    if (download_requested()) {
      ...
      isolate({
        fmt <- switch(input$format,
          PDF = list(input = "pdf_9.qmd", format = "pdf", ext = "pdf"),
