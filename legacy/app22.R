...
server <- function(input, output, session) {
  ...
  output$plotA_inputs_ui <- renderUI({
    ...
    # Check if we're currently generating UI for Plot A
    if (plotA_ui_generation_in_progress()) {
      return(div(
        style = "text-align: center; padding: 20px;",
        tags$i(class = "fa fa-spinner fa-spin", style = "font-size: 24px; color: #cc4c02;"),
        br(), br(),
        p("Generating inputs...", style = "color: #666;")
      ))
    }
    
    # Generate UI synchronously (crew was causing issues with renderUI)
    plot_inputs <- lapply(1:input$plotA_n, function(i) {
      div(
        if (i > 1) br(),
        selectInput(paste0("plotA_x_", i), paste("X Variable for", PLOT_A_LABEL, i, ":"), 
                   choices = names(mtcars), 
                   selected = if (i == 1) "wt" else if (i == 2) "hp" else "disp"),
        h6(paste("Sub-Caption for", PLOT_A_LABEL, i), class = "text-secondary", style = "font-weight: bold;"),
    ...
    shared_inputs <- div(
      ...
      textInput("plotA_caption_shared", paste("Shared Caption for All", PLOT_A_LABEL), 
               placeholder = paste("Overall caption for the", PLOT_A_LABEL, "figure group")),
      textAreaInput("plotA_notes_shared", paste("Notes for", PLOT_A_LABEL, "Plots"), 
  ...
  # Plot B dynamic inputs (with stability improvements)
  output$plotB_inputs_ui <- renderUI({
    ...
    # Generate individual plot inputs with stable structure
    plot_inputs <- lapply(1:input$plotB_n, function(i) {
      div(
        key = paste0("plotB_input_", i),  # Add key for stability
        if (i > 1) br(),
        div(
          id = paste0("plotB_select_container_", i),  # Stable container
          selectInput(
            inputId = paste0("plotB_x_", i), 
        ...
        textInput(
          inputId = paste0("plotB_SubCaption_", i), 
          label = NULL, 
          placeholder = "Brief description of this plot"
        )
      )
    ...
    shared_inputs <- div(
      key = "plotB_shared_inputs",  # Stable key
      br(),
    ...
    # Return with a stable wrapper
    return(div(
      id = paste0("plotB_wrapper_", input$plotB_n),
      plot_inputs, 
      shared_inputs
    ))
  })
    ...
    local({
      ...
      output[[paste0("regPlotA_", my_i)]] <- renderPlot({
        ...
        ggplot(mtcars, aes_string(x = input[[paste0("plotA_x_", my_i)]], y = "mpg")) +
          ...
          labs(title = paste(PLOT_A_LABEL, my_i), x = input[[paste0("plotA_x_", my_i)]], y = "mpg") +
          theme(
    ...
    local({
      ...
      output[[paste0("regPlotB_", my_i)]] <- renderPlot({
        ...
        ggplot(mtcars, aes_string(x = input[[paste0("plotB_x_", my_i)]], y = "mpg")) +
          ...
          geom_smooth(method = "lm", se = FALSE, color = color_secondary) +
          theme_minimal(base_family = "Helvetica Neue") +
          labs(title = paste(PLOT_B_LABEL, my_i), x = input[[paste0("plotB_x_", my_i)]], y = "mpg") +
          theme(
            plot.background = element_rect(fill = color_panel_bg, color = NA),
            panel.background = element_rect(fill = color_panel_bg, color = NA),
            text = element_text(color = color_fg),
