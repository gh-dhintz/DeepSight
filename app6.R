...
server <- function(input, output, session) {
  ...
  # Function to extract relevant parameters for comparison (metadata and plot vars)
  extractReportParams <- function(param_list, max_plots = 3) {
    # Metadata parameters (what used to be execute_params)
    metadata_params <- c("title", "subtitle", "name")
    
    # Plot configuration variables (everything else)
    plot_vars <- c(
    ...
    all_relevant_params <- c(metadata_params, plot_vars)
...
observeEvent(input$load_report_1, {
  ...
  output$comparisonTable <- renderTable({
    ...
    for (param in all_params) {
      ...
      # Create readable parameter names (separated metadata vs plot variables)
