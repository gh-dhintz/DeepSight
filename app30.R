...
initialize_log_file <- function() {
  if (!file.exists(REPORT_LOG_FILE)) {
    ...
    # Plot A - Annotation conifig
    for (i in 1:MAX_PLOTS_A) {
      header_cols <- c(header_cols, 
                      paste0("plot_A_x", i),
                      paste0("plot_A_SubCaption", i),
                      paste0("plot_A_SubCaption_alternate_1_", i),
                      paste0("plot_A_SubCaption_alternate_2_", i),
                      paste0("plot_A_SubCaption_alternate_3_", i),
                      paste0("plot_A_SubCaption_alternate_4_", i),
                      paste0("plot_A_SubCaption_alternate_5_", i),
                      paste0("plot_A_SubCaption_alternate_6_", i),
                      paste0("plot_A_SubCaption_alternate_7_", i))
    }
...
log_report_generation <- function(all_inputs, format, bookmark_url) {
  ...
  for (i in 1:MAX_PLOTS_A) {
    ...
    # Plot A - Annotation conifig
    subcaption_alt1_name <- paste0("plot_A_SubCaption_alternate_1_", i)
    subcaption_alt2_name <- paste0("plot_A_SubCaption_alternate_2_", i)
    subcaption_alt3_name <- paste0("plot_A_SubCaption_alternate_3_", i)
    subcaption_alt4_name <- paste0("plot_A_SubCaption_alternate_4_", i)
    subcaption_alt5_name <- paste0("plot_A_SubCaption_alternate_5_", i)
    subcaption_alt6_name <- paste0("plot_A_SubCaption_alternate_6_", i)
    subcaption_alt7_name <- paste0("plot_A_SubCaption_alternate_7_", i)
    ...
    log_entry[[subcaption_alt1_name]] <- ifelse(is.null(all_inputs[[subcaption_alt1_name]]), "", all_inputs[[subcaption_alt1_name]])
    log_entry[[subcaption_alt2_name]] <- ifelse(is.null(all_inputs[[subcaption_alt2_name]]), "", all_inputs[[subcaption_alt2_name]])
    log_entry[[subcaption_alt3_name]] <- ifelse(is.null(all_inputs[[subcaption_alt3_name]]), "", all_inputs[[subcaption_alt3_name]])
    log_entry[[subcaption_alt4_name]] <- ifelse(is.null(all_inputs[[subcaption_alt4_name]]), "", all_inputs[[subcaption_alt4_name]])
    log_entry[[subcaption_alt5_name]] <- ifelse(is.null(all_inputs[[subcaption_alt5_name]]), "", all_inputs[[subcaption_alt5_name]])
    log_entry[[subcaption_alt6_name]] <- ifelse(is.null(all_inputs[[subcaption_alt6_name]]), "", all_inputs[[subcaption_alt6_name]])
    log_entry[[subcaption_alt7_name]] <- ifelse(is.null(all_inputs[[subcaption_alt7_name]]), "", all_inputs[[subcaption_alt7_name]])
  }
...
output$plot_A_inputs_ui <- renderUI({
  ...
  style_tag <- tags$style(HTML("
    ...
    input[id*='plot_A_SubCaption_alternate_']::placeholder {
  ...
  plot_inputs <- lapply(1:plot_count_trigger, function(i) {
    ...
    current_subcaption_alt1 <- isolate(input[[subcaption_alt1_name]])
    current_subcaption_alt2 <- isolate(input[[subcaption_alt2_name]])
    current_subcaption_alt3 <- isolate(input[[subcaption_alt3_name]])
    current_subcaption_alt4 <- isolate(input[[subcaption_alt4_name]])
    current_subcaption_alt5 <- isolate(input[[subcaption_alt5_name]])
    current_subcaption_alt6 <- isolate(input[[subcaption_alt6_name]])
    current_subcaption_alt7 <- isolate(input[[subcaption_alt7_name]])
    ...
    div(
      ...
      # Plot A - Annotation conifig
      h6(paste0("Plot Annotations for ", PLOT_A_LABEL, " (", i, ")"), 
    class = "text-secondary", style = "font-weight: bold;"),
      textInput(subcaption_alt1_name, 
              div("Title:", style = "font-size: 12px; font-weight: bold;"),
              value = if(!is.null(current_subcaption_alt1)) current_subcaption_alt1 else "",
              placeholder = "Alternative description 1"),
      textInput(subcaption_alt2_name,
              div("Subtitle:", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_subcaption_alt2)) current_subcaption_alt2 else "",
              placeholder = "Alternative description 2"),
      textInput(subcaption_alt3_name, 
              div("Alt Caption 3:", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_subcaption_alt3)) current_subcaption_alt3 else "",
              placeholder = "Alternative description 3"),
      textInput(subcaption_alt4_name, 
              div("Alt Caption 4:", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_subcaption_alt4)) current_subcaption_alt4 else "",
              placeholder = "Alternative description 4"),
      textInput(subcaption_alt5_name,
              div("Alt Caption 5:", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_subcaption_alt5)) current_subcaption_alt5 else "",
              placeholder = "Alternative description 5"),
      textInput(subcaption_alt6_name, 
              div("Alt Caption 6:", style = "font-size: 11px; font-weight: bold;"),
              value = if(!is.null(current_subcaption_alt6)) current_subcaption_alt6 else "",
              placeholder = "Alternative description 6"),
      textInput(subcaption_alt7_name, 
      div("Alt Caption 7:", style = "font-size: 11px; font-weight: bold;"),
      value = if(!is.null(current_subcaption_alt7)) current_subcaption_alt7 else "",
      placeholder = "Alternative description 7")
    )
  })
...
text_and_dropdown_inputs <- reactive({
  ...
  if (!is.null(input$plot_A_n) && input$plot_A_n > 0) {
    for (i in 1:input$plot_A_n) {
      ...
      all_vals[[subcaption_alt1_name]] <- input[[subcaption_alt1_name]]
      all_vals[[subcaption_alt2_name]] <- input[[subcaption_alt2_name]]
      all_vals[[subcaption_alt3_name]] <- input[[subcaption_alt3_name]]
      all_vals[[subcaption_alt4_name]] <- input[[subcaption_alt4_name]]
      all_vals[[subcaption_alt5_name]] <- input[[subcaption_alt5_name]]
      all_vals[[subcaption_alt6_name]] <- input[[subcaption_alt6_name]]
      all_vals[[subcaption_alt7_name]] <- input[[subcaption_alt7_name]]
    }
  }
  ...
  loadPresetReport <- function(preset_url) {
    ...
    if (length(extracted) > 0) {
      ...
      observe({
        ...
        update_dynamic_inputs("plot_A_SubCaption_alternate_1_", "text")
        update_dynamic_inputs("plot_A_SubCaption_alternate_2_", "text")
        update_dynamic_inputs("plot_A_SubCaption_alternate_3_", "text")
        update_dynamic_inputs("plot_A_SubCaption_alternate_4_", "text")
        update_dynamic_inputs("plot_A_SubCaption_alternate_5_", "text")
        update_dynamic_inputs("plot_A_SubCaption_alternate_6_", "text")
        update_dynamic_inputs("plot_A_SubCaption_alternate_7_", "text")
  ...
  if (copy_after_bookmark()) {
    ...
    if (download_requested()) {
      ...
      plot_patterns <- c("^plot_A_x", "^plot_B_x", "^plot_A_SubCaption", 
                   "^plot_A_SubCaption_alternate_1_", "^plot_A_SubCaption_alternate_2_",
                   "^plot_A_SubCaption_alternate_3_", "^plot_A_SubCaption_alternate_4_",
                   "^plot_A_SubCaption_alternate_5_", "^plot_A_SubCaption_alternate_6_", "^plot_A_SubCaption_alternate_7_",
                   "^plot_B_SubCaption")
      ...
      for (plot_type in c("plot_A", "plot_B")) {
        ...
        if (!is.null(n_plots) && n_plots > 0) {
          ...
          # Plot A - Annotation conifig
          subcaption_alt1_name <- paste0(plot_type, "_SubCaption_alternate_1_", i)
          subcaption_alt2_name <- paste0(plot_type, "_SubCaption_alternate_2_", i)
          subcaption_alt3_name <- paste0(plot_type, "_SubCaption_alternate_3_", i)
          subcaption_alt4_name <- paste0(plot_type, "_SubCaption_alternate_4_", i)
          subcaption_alt5_name <- paste0(plot_type, "_SubCaption_alternate_5_", i)
          subcaption_alt6_name <- paste0(plot_type, "_SubCaption_alternate_6_", i)
          subcaption_alt7_name <- paste0(plot_type, "_SubCaption_alternate_7_", i)
          
          if (!is.null(input[[subcaption_alt1_name]])) {
            all_inputs[[subcaption_alt1_name]] <- input[[subcaption_alt1_name]]
          }
          if (!is.null(input[[subcaption_alt2_name]])) {
            all_inputs[[subcaption_alt2_name]] <- input[[subcaption_alt2_name]]
          }
          if (!is.null(input[[subcaption_alt3_name]])) {
            all_inputs[[subcaption_alt3_name]] <- input[[subcaption_alt3_name]]
          }
          if (!is.null(input[[subcaption_alt4_name]])) {
            all_inputs[[subcaption_alt4_name]] <- input[[subcaption_alt4_name]]
          }
          if (!is.null(input[[subcaption_alt5_name]])) {
            all_inputs[[subcaption_alt5_name]] <- input[[subcaption_alt5_name]]
          }
          if (!is.null(input[[subcaption_alt6_name]])) {
            all_inputs[[subcaption_alt6_name]] <- input[[subcaption_alt6_name]]
          }
          if (!is.null(input[[subcaption_alt7_name]])) {
            all_inputs[[subcaption_alt7_name]] <- input[[subcaption_alt7_name]]
          }
        }
...
for (i in 1:MAX_PLOTS_A) {
  local({
    ...
    output[[paste0("Plot_A_Obj", my_i)]] <- renderPlot({
      ...
      title_trigger <- input[[paste0("plot_A_SubCaption_alternate_1_", my_i)]]
      my_title <- if(!is.null(title_trigger) && title_trigger != "") {
        title_trigger
      } else {
        paste0(PLOT_A_LABEL, " (", my_i, ")")
      }
      subtitle_trigger <- input[[paste0("plot_A_SubCaption_alternate_2_", my_i)]]
      my_subtitle <- if(!is.null(subtitle_trigger) && subtitle_trigger != "") {
        subtitle_trigger
      } else {
        ""
      }
      
      final_caption <- current_subcaption
      ...
      if (section_visited && !is.null(input[[paste0(plot_type, "_n")]]) && input[[paste0(plot_type, "_n")]] > 0) {
        ...
        for (i in 1:n_plots) {
          ...
          if (!is.null(x_var)) {
            ...
            # Add all 6 alternate captions
            # Plot A - Annotation conifig
            caption_alt1_var <- input[[paste0(plot_type, "_SubCaption_alternate_1_", i)]]
            caption_alt2_var <- input[[paste0(plot_type, "_SubCaption_alternate_2_", i)]]
            caption_alt3_var <- input[[paste0(plot_type, "_SubCaption_alternate_3_", i)]]
            caption_alt4_var <- input[[paste0(plot_type, "_SubCaption_alternate_4_", i)]]
            caption_alt5_var <- input[[paste0(plot_type, "_SubCaption_alternate_5_", i)]]
            caption_alt6_var <- input[[paste0(plot_type, "_SubCaption_alternate_6_", i)]]
            caption_alt7_var <- input[[paste0(plot_type, "_SubCaption_alternate_7_", i)]]

            # Plot A - Annotation conifig
            if (!is.null(caption_alt1_var) && caption_alt1_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Alt Caption 1"),
                Value = if(nchar(caption_alt1_var) > 50) paste0(substr(caption_alt1_var, 1, 50), "...") else caption_alt1_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(caption_alt2_var) && caption_alt2_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Alt Caption 2"),
                Value = if(nchar(caption_alt2_var) > 50) paste0(substr(caption_alt2_var, 1, 50), "...") else caption_alt2_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(caption_alt3_var) && caption_alt3_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Alt Caption 3"),
                Value = if(nchar(caption_alt3_var) > 50) paste0(substr(caption_alt3_var, 1, 50), "...") else caption_alt3_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(caption_alt4_var) && caption_alt4_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Alt Caption 4"),
                Value = if(nchar(caption_alt4_var) > 50) paste0(substr(caption_alt4_var, 1, 50), "...") else caption_alt4_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(caption_alt5_var) && caption_alt5_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Alt Caption 5"),
                Value = if(nchar(caption_alt5_var) > 50) paste0(substr(caption_alt5_var, 1, 50), "...") else caption_alt5_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(caption_alt6_var) && caption_alt6_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Alt Caption 6"),
                Value = if(nchar(caption_alt6_var) > 50) paste0(substr(caption_alt6_var, 1, 50), "...") else caption_alt6_var,
                stringsAsFactors = FALSE
              ))
            }
            if (!is.null(caption_alt7_var) && caption_alt7_var != "") {
              plot_details <<- rbind(plot_details, data.frame(
                Setting = paste0(plot_label, ".", i, " - Alt Caption 7"),
                Value = if(nchar(caption_alt6_var) > 50) paste0(substr(caption_alt7_var, 1, 50), "...") else caption_alt7_var,
                stringsAsFactors = FALSE
              ))
            }
          }
  ...
  extractReportParams <- function(param_list) {
    ...
    dynamic_patterns <- c("^plot_A_x", "^plot_B_x", "^plot_A_SubCaption", 
                      "^plot_A_SubCaption_alternate_1_", "^plot_A_SubCaption_alternate_2_",
                      "^plot_A_SubCaption_alternate_3_", "^plot_A_SubCaption_alternate_4_",
                      "^plot_A_SubCaption_alternate_5_", "^plot_A_SubCaption_alternate_6_", "^plot_A_SubCaption_alternate_7_",
                      "^plot_B_SubCaption")
  ...
  create_readable_param_name <- function(param) {
    ...
    # Plot A - Annotation conifig
    if (grepl("^plot_A_SubCaption_alternate_1_", param)) {
      plot_num <- gsub("plot_A_SubCaption_alternate_1_", "", param)
      return(paste0("📊 ", PLOT_A_LABEL, " ", plot_num, " - Alt Sub-Caption 1"))
    }
    if (grepl("^plot_A_SubCaption_alternate_2_", param)) {
      plot_num <- gsub("plot_A_SubCaption_alternate_2_", "", param)
      return(paste0("📊 ", PLOT_A_LABEL, " ", plot_num, " - Alt Sub-Caption 2"))
    }
    if (grepl("^plot_A_SubCaption_alternate_3_", param)) {
      plot_num <- gsub("plot_A_SubCaption_alternate_3_", "", param)
      return(paste0("📊 ", PLOT_A_LABEL, " ", plot_num, " - Alt Sub-Caption 3"))
    }
    if (grepl("^plot_A_SubCaption_alternate_4_", param)) {
      plot_num <- gsub("plot_A_SubCaption_alternate_4_", "", param)
      return(paste0("📊 ", PLOT_A_LABEL, " ", plot_num, " - Alt Sub-Caption 4"))
    }
    if (grepl("^plot_A_SubCaption_alternate_5_", param)) {
      plot_num <- gsub("plot_A_SubCaption_alternate_5_", "", param)
      return(paste0("📊 ", PLOT_A_LABEL, " ", plot_num, " - Alt Sub-Caption 5"))
    }
    if (grepl("^plot_A_SubCaption_alternate_6_", param)) {
      plot_num <- gsub("plot_A_SubCaption_alternate_6_", "", param)
      return(paste0("📊 ", PLOT_A_LABEL, " ", plot_num, " - Alt Sub-Caption 6"))
    }
    if (grepl("^plot_A_SubCaption_alternate_7_", param)) {
      plot_num <- gsub("plot_A_SubCaption_alternate_7_", "", param)
      return(paste0("📊 ", PLOT_A_LABEL, " ", plot_num, " - Alt Sub-Caption 7"))
    }
  ...
  load_report_params <- function(params) {
    if (length(params) > 0) {
      ...
      observe({
        ...
        update_dynamic_inputs("plot_A_x", "select")
        update_dynamic_inputs("plot_A_SubCaption", "text")
        update_dynamic_inputs("plot_A_SubCaption_alternate", "text")
        update_dynamic_inputs("plot_B_x", "select")
        update_dynamic_inputs("plot_B_SubCaption", "text")
