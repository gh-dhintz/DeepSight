...
PLOT_A_LABEL <- "My First Plot"   # Display name for Plot A everywhere
PLOT_B_LABEL <- "My Second Plot"  # Display name for Plot B everywhere
          ...
          tabPanel(PLOT_A_LABEL,
            sidebarLayout(
              sidebarPanel(width = 3,
                ...
                numericInput("plotA_n", paste0("Number of plots (up to ", MAX_PLOTS_A ,"):"), value = 1, min = 1, max = MAX_PLOTS_A),
          ...
          tabPanel(PLOT_B_LABEL,
            sidebarLayout(
              sidebarPanel(width = 3,
                ...
                numericInput("plotB_n", paste0("Number of plots (up to ", MAX_PLOTS_B ,"):"), value = 1, min = 1, max = MAX_PLOTS_B),
...
initialize_log_file <- function() {
  if (!file.exists(REPORT_LOG_FILE)) {
    ...
    header_cols <- c(
      ...
      "plotA_n", "plotB_n", 
      "plotA_caption_shared", "plotA_notes_shared",
    ...
    for (i in 1:MAX_PLOTS_A) {
      header_cols <- c(header_cols, 
                      paste0("plotA_x_", i),
                      paste0("plotA_SubCaption_", i))
    }
    
    for (i in 1:MAX_PLOTS_B) {
      header_cols <- c(header_cols,
                      paste0("plotB_x_", i), 
                      paste0("plotB_SubCaption_", i))
    }
...
log_report_generation <- function(all_inputs, format, bookmark_url) {
  ...
  log_entry$plotA_n <- ifelse(is.null(all_inputs$plotA_n), "", all_inputs$plotA_n)
  log_entry$plotB_n <- ifelse(is.null(all_inputs$plotB_n), "", all_inputs$plotB_n)
  log_entry$plotA_caption_shared <- ifelse(is.null(all_inputs$plotA_caption_shared), "", all_inputs$plotA_caption_shared)
  log_entry$plotA_notes_shared <- ifelse(is.null(all_inputs$plotA_notes_shared), "", all_inputs$plotA_notes_shared)
  log_entry$plotB_caption_shared <- ifelse(is.null(all_inputs$plotB_caption_shared), "", all_inputs$plotB_caption_shared)
  log_entry$plotB_notes_shared <- ifelse(is.null(all_inputs$plotB_notes_shared), "", all_inputs$plotB_notes_shared)
...
read_log_file <- function() {
  ...
  plotA_ui_generation_in_progress <- reactiveVal(FALSE)
  plotB_ui_generation_in_progress <- reactiveVal(FALSE)
...
output$plotA_inputs_ui <- renderUI({
  ...
  plot_count_trigger <- input$plotA_n
  ...
  current_caption <- isolate(input[["plotA_caption_shared"]])
  current_notes <- isolate(input[["plotA_notes_shared"]])
  
  shared_inputs <- div(
    br(),
    h5(paste("Shared Settings for All", PLOT_A_LABEL), class = "text-info"),
    textInput("plotA_caption_shared", 
    ...
    textAreaInput("plotA_notes_shared", 
                 paste("Notes for", PLOT_A_LABEL, "Plots"), 
                 value = if(!is.null(current_notes)) current_notes else "",
                 placeholder = paste("Add your comments about", PLOT_A_LABEL, "here. When referencing in text, use @fig-plotA when nplots=1; otherwise, use @fig-plotA-1, @fig-plotA-2, etc."), 
                 height = "100px", width = "100%")
  )
...
output$plotB_inputs_ui <- renderUI({
  ...
  # Create a reactive trigger that only changes when plot count changes
  plot_count_trigger <- input$plotB_n
  ...
  plot_inputs <- lapply(1:plot_count_trigger, function(i) {
    ...
    div(
      key = paste0("plotB_input_", i),
      if (i > 1) br(),
      div(
        id = paste0("plotB_select_container_", i),
        selectInput(
          inputId = x_var_name, 
  ...
  # PRESERVE SHARED INPUT VALUES with isolate()
  current_caption <- isolate(input[["plotB_caption_shared"]])
  current_notes <- isolate(input[["plotB_notes_shared"]])
  
  shared_inputs <- div(
    key = "plotB_shared_inputs",
    br(),
    h5(paste("Shared Settings for All", PLOT_B_LABEL), class = "text-info"),
    textInput("plotB_caption_shared", 
             paste("Shared Caption for All", PLOT_B_LABEL), 
             value = if(!is.null(current_caption)) current_caption else "",
             placeholder = paste("Overall caption for the", PLOT_B_LABEL, "figure group")),
    textAreaInput("plotB_notes_shared", 
                 paste("Notes for", PLOT_B_LABEL, "Plots"), 
                 value = if(!is.null(current_notes)) current_notes else "",
                 placeholder = paste("Add your comments about", PLOT_B_LABEL, "here. When referencing in text, use @fig-plotB when nplots=1; otherwise, use @fig-plotB-1, @fig-plotB-2, etc."), 
                 height = "100px", 
                 width = "100%")
  )
...
text_and_dropdown_inputs <- reactive({
  ...
  all_vals$plotA_caption_shared <- input$plotA_caption_shared
  all_vals$plotA_notes_shared <- input$plotA_notes_shared
  all_vals$plotB_caption_shared <- input$plotB_caption_shared
  all_vals$plotB_notes_shared <- input$plotB_notes_shared
...
observeEvent(text_debounced(), {
  ...
  observeEvent(input$load_basic_report, {
    preset_url <- "http://127.0.0.1:4001/?_inputs_&nav=%22Report%20Export%22&plots_subtabs=%22My%20Second%20Plot%22&load_basic_report=0&load_basic_report_two=0&update_and_copy_url=0&download_trigger=1&parse_url_1=0&parse_url_2=0&load_report_1=0&load_report_2=0&clear_comparison=0&refresh_log=0&clear_log=0&format=%22PDF%22&plotA_n=1&plotB_n=1&bookmark_url_1=%22%22&bookmark_url_2=%22%22&title=%22My%20Report%20Title%22&subtitle=%22Analysis%20Report%22&name=%22John%20Doe%22&plotA_x_1=%22wt%22&plotA_notes_shared=%22%22&plotA_SubCaption_1=%22%22&plotA_caption_shared=%22%22&plotB_x_1=%22wt%22&plotB_notes_shared=%22%22&plotB_SubCaption_1=%22%22&plotB_caption_shared=%22%22"
...
observeEvent(input$update_and_copy_url, {
    ...
    local({
      ...
      output[[paste0("regPlotA_", my_i)]] <- renderPlot({
        ...
        ggplot(mtcars, aes_string(x = input[[paste0("plotA_x_", my_i)]], y = "mpg")) +
          ...
          labs(title = paste0(PLOT_A_LABEL, " (", my_i, ")"), x = input[[paste0("plotA_x_", my_i)]], y = "mpg") +
          theme(
    ...
    local({
      ...
      output[[paste0("regPlotB_", my_i)]] <- renderPlot({
        ...
        ggplot(mtcars, aes_string(x = input[[paste0("plotB_x_", my_i)]], y = "mpg")) +
          ...
          geom_smooth(method = "lm", se = FALSE, color = color_secondary) +
          theme_minimal(base_family = "Helvetica Neue") +
          labs(title = paste0(PLOT_B_LABEL, " (", my_i, ")"), x = input[[paste0("plotB_x_", my_i)]], y = "mpg") +
          theme(
            plot.background = element_rect(fill = color_panel_bg, color = NA),
            panel.background = element_rect(fill = color_panel_bg, color = NA),
            text = element_text(color = color_fg),
  ...
  output$checklistTable <- renderTable({
    ...
    checklist_data <- data.frame(
      ...
      Description = c(
        if(plotA_visited()) paste(PLOT_A_LABEL, "section has been configured") else paste("Please visit", PLOT_A_LABEL, "tab to configure plots"),
        if(plotB_visited()) paste(PLOT_B_LABEL, "section has been configured") else paste("Please visit", PLOT_B_LABEL, "tab to configure plots")
      ),
  ...
  output$inputPreviewTable <- renderTable({
    ...
    add_plot_details <- function(plot_type, section_visited) {
      if (section_visited && !is.null(input[[paste0(plot_type, "_n")]]) && input[[paste0(plot_type, "_n")]] > 0) {
        ...
        # Create proper plot type label (plotA → Plot A, plotB → Plot B)
        plot_label <- if (plot_type == "plotA") PLOT_A_LABEL else if (plot_type == "plotB") PLOT_B_LABEL else plot_type
  ...
  create_readable_param_name <- function(param) {
    ...
    if (param == "plotA_n") return(paste0("📊 ", PLOT_A_LABEL, " - Number of Plots"))
    if (param == "plotB_n") return(paste0("📊 ", PLOT_B_LABEL, " - Number of Plots"))
    if (param == "plotA_caption_shared") return(paste0("📊 ", PLOT_A_LABEL, " - Shared Caption"))
    if (param == "plotA_notes_shared") return(paste0("📊 ", PLOT_A_LABEL, " - Shared Notes"))
    if (param == "plotB_caption_shared") return(paste0("📊 ", PLOT_B_LABEL, " - Shared Caption"))
    if (param == "plotB_notes_shared") return(paste0("📊 ", PLOT_B_LABEL, " - Shared Notes"))
