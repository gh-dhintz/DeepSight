...
server <- function(input, output, session) {
    ...
    if (download_requested()) {
      ...
      isolate({
        fmt <- switch(input$format,
          PDF = list(input = "pdf_new.qmd", format = "pdf", ext = "pdf"),
        ...
        tryCatch({
          withProgress(message = "Rendering report", value = 0.3, {
            ...
            quarto::quarto_render(
              ...
              #execute_params = list(title = input$title, subitle= input$subitle, name = input$name), # will incorperate passing params to yaml header later
              execute_dir = getwd()
            )
  ...
  # Function to extract relevant parameters for comparison (removed shared label params, added shared captions)
  extractReportParams <- function(param_list, max_plots = 3) {
    relevant_params <- c(
                          ...
                          paste0("plotA_SubCaption_", 1:max_plots),
                          paste0("plotB_SubCaption_", 1:max_plots)
                        )
  ...
  ##### Load Report 1 parameters into current session (removed shared label updates) #####
observeEvent(input$load_report_1, {
  ...
  if (length(params) > 0) {
    ...
    observe({
      ...
      ## Update caption ------------------------------------------------------------------#
      if (!is.na(params$plotA_SubCaption_1) && !is.null(input$plotA_SubCaption_1)) {
        updateTextInput(session, "plotA_SubCaption_1", value = params$plotA_SubCaption_1)
      }
      if (!is.na(params$plotA_SubCaption_2) && !is.null(input$plotA_SubCaption_2)) {
        updateTextInput(session, "plotA_SubCaption_2", value = params$plotA_SubCaption_2)
      }
      if (!is.na(params$plotA_SubCaption_3) && !is.null(input$plotA_SubCaption_3)) {
        updateTextInput(session, "plotA_SubCaption_3", value = params$plotA_SubCaption_3)
      }
      ## Update shared caption --------------------------------------------------------------#
      if (!is.na(params$plotA_caption_shared) && !is.null(input$plotA_caption_shared)) {
        updateTextInput(session, "plotA_caption_shared", value = params$plotA_caption_shared)
      }
      ## Update notes --------------------------------------------------------------------#
      ...
      ## Update caption ------------------------------------------------------------------#
      if (!is.na(params$plotB_SubCaption_1) && !is.null(input$plotB_SubCaption_1)) {
        updateTextInput(session, "plotB_SubCaption_1", value = params$plotB_SubCaption_1)
      }
      if (!is.na(params$plotB_SubCaption_2) && !is.null(input$plotB_SubCaption_2)) {
        updateTextInput(session, "plotB_SubCaption_2", value = params$plotB_SubCaption_2)
      }
      if (!is.na(params$plotB_SubCaption_3) && !is.null(input$plotB_SubCaption_3)) {
        updateTextInput(session, "plotB_SubCaption_3", value = params$plotB_SubCaption_3)
      }
      ## Update shared caption --------------------------------------------------------------#
      if (!is.na(params$plotB_caption_shared) && !is.null(input$plotB_caption_shared)) {
        updateTextInput(session, "plotB_caption_shared", value = params$plotB_caption_shared)
      }
      ## Update notes --------------------------------------------------------------------#
  ...
  ##### Load Report 2 parameters into current session (removed shared label updates) #####
  observeEvent(input$load_report_2, {
    params <- report2_params()  # Fixed: was using report1_params() instead of report2_params()
    if (length(params) > 0) {
      ...
      observe({
        ...
        if (!is.na(params$plotA_SubCaption_1) && !is.null(input$plotA_SubCaption_1)) {
          updateTextInput(
            session,
            "plotA_SubCaption_1",
            value = params$plotA_SubCaption_1
          )
        }
        if (!is.na(params$plotA_SubCaption_2) && !is.null(input$plotA_SubCaption_2)) {
          updateTextInput(
            session,
            "plotA_SubCaption_2",
            value = params$plotA_SubCaption_2
          )
        }
        if (!is.na(params$plotA_SubCaption_3) && !is.null(input$plotA_SubCaption_3)) {
          updateTextInput(
            session,
            "plotA_SubCaption_3",
            value = params$plotA_SubCaption_3
          )
        }
        ## Update shared caption --------------------------------------------------------------#
        if (
          !is.na(params$plotA_caption_shared) &&
            !is.null(input$plotA_caption_shared)
        ) {
          updateTextInput(
            session,
            "plotA_caption_shared",
            value = params$plotA_caption_shared
          )
        }
        ## Update notes --------------------------------------------------------------------#
        if (
          !is.na(params$plotA_notes_shared) &&
            !is.null(input$plotA_notes_shared)
        ) {
          updateTextAreaInput(
            session,
            "plotA_notes_shared",
            value = params$plotA_notes_shared
          )
        }
        # Update Plot B Inputs -------------------------------------------------------------#
        ## Update x ------------------------------------------------------------------------#
        ...
        if (!is.na(params$plotB_SubCaption_1) && !is.null(input$plotB_SubCaption_1)) {
          updateTextInput(
            session,
            "plotB_SubCaption_1",
            value = params$plotB_SubCaption_1
          )
        }
        if (!is.na(params$plotB_SubCaption_2) && !is.null(input$plotB_SubCaption_2)) {
          updateTextInput(
            session,
            "plotB_SubCaption_2",
            value = params$plotB_SubCaption_2
          )
        }
        if (!is.na(params$plotB_SubCaption_3) && !is.null(input$plotB_SubCaption_3)) {
          updateTextInput(
            session,
            "plotB_SubCaption_3",
            value = params$plotB_SubCaption_3
          )
        }
        ## Update shared caption --------------------------------------------------------------#
        if (
          !is.na(params$plotB_caption_shared) &&
            !is.null(input$plotB_caption_shared)
        ) {
          updateTextInput(
            session,
            "plotB_caption_shared",
            value = params$plotB_caption_shared
          )
        }
        ## Update notes --------------------------------------------------------------------#
        if (
          !is.na(params$plotB_notes_shared) &&
            !is.null(input$plotB_notes_shared)
        ) {
          updateTextAreaInput(
            session,
            "plotB_notes_shared",
            value = params$plotB_notes_shared
          )
        }
      })
  ...
  output$comparisonTable <- renderTable({
    ...
    for (param in all_params) {
      ...
      readable_param <- switch(param,
        ...
        "plotA_SubCaption_1" = "Plot A 1 - Sub-Caption",
        "plotA_SubCaption_2" = "Plot A 2 - Sub-Caption",
        "plotA_SubCaption_3" = "Plot A 3 - Sub-Caption",
        "plotA_caption_shared" = "Plot A - Shared Caption",
        "plotA_notes_shared" = "Plot A - Shared Notes",
        "plotB_SubCaption_1" = "Plot B 1 - Sub-Caption",
        "plotB_SubCaption_2" = "Plot B 2 - Sub-Caption",
        "plotB_SubCaption_3" = "Plot B 3 - Sub-Caption",
        "plotB_caption_shared" = "Plot B - Shared Caption",
