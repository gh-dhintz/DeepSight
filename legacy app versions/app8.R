  ...
  navbarPage(
    ...
    tabPanel("Preload Reports",
      ...
      fluidRow(
        column(8,
          div(
            ...
            h5("Report A: ESR1 Alone", class = "text-info", style = "margin-top: 0;"),
            p("A sample report with minimal configuration", 
              style = "margin-bottom: 10px; color: #666;")
          )
        ),
        column(4,
          div(style = "padding-top: 30px;",
            actionButton("load_basic_report", "Load Report A", 
      ...
      fluidRow(
        column(8,
          div(
            class = "panel panel-default",
            style = "padding: 15px; margin-bottom: 15px; border: 1px solid #ddd; border-radius: 5px;",
            h5("Report B: ESR1 Vs =PIK3ca", class = "text-info", style = "margin-top: 0;"),
            p("Cupidatat velit ea qui. In proident laborum Lorem enim irure dolor velit incididunt sint cupidatat dolore et cillum duis.", 
              style = "margin-bottom: 10px; color: #666;")
          )
        ),
        column(4,
          div(style = "padding-top: 30px;",
            actionButton("load_report_B", "Load Report B", 
                        class = "btn btn-primary btn-lg", 
                        style = "width: 100%;")
          )
        )
      ...
      div(
        ...
        p("These preload buttons will automatically configure all the plot settings, captions, and metadata based on predefined templates. After loading, you can modify any settings in the Plots, or User Guide tabs before generating your report.", 
          style = "margin-bottom: 0; font-size: 14px;")
      )
    ),
    ##### Plot A #####
...
server <- function(input, output, session) {
  ...
  # Load Report A
  observeEvent(input$load_report_A, {
    ...
    showNotification("Report A configuration loaded successfully!", type = "message")
  })
  
  # Load Basic Report Two
  observeEvent(input$load_report_B, {
    preset_url <- "http://127.0.0.1:6988/?_inputs_&nav=%22Report%20Export%22&load_basic_report=1&load_basic_report_two=0&download_trigger=1&parse_url_1=0&parse_url_2=0&load_report_1=0&load_report_2=0&clear_comparison=0&format=%22PDF%22&plotA_n=3&plotB_n=1&bookmark_url_1=%22%22&bookmark_url_2=%22%22&title=%22My%20Report%20Title%22&subtitle=%22Analysis%20Report%22&name=%22John%20Doe%22&plotA_x_1=%22mpg%22&plotA_notes_shared=%22Nice%20plots%22&plotA_SubCaption_1=%22foo%22&plotA_caption_shared=%22see%20overall%22&plotA_x_2=%22hp%22&plotA_SubCaption_2=%22bah%22&plotA_x_3=%22drat%22&plotA_SubCaption_3=%22baz%22&plotB_x_1=%22mpg%22&plotB_notes_shared=%22%22&plotB_SubCaption_1=%22%22&plotB_caption_shared=%22%22"
    
    loadPresetReport(preset_url)
    showNotification("Report B configuration loaded successfully!", type = "message")
  })
