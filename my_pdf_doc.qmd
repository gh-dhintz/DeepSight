...
cat("xvarA:", params$xvarA, "\n")
fml <- as.formula(paste("mpg ~", params$xvarA))
...
cat("xvarB:", params$xvarB, "\n")
fml <- as.formula(paste("mpg ~", params$xvarB))
fit <- lm(fml, data = mtcars)
b <- coef(fit)
summary(fit)
```

The fitting result is $mpg = `r b[1]` + `r b[2]``r params$xvarB`$.
Below are scatter plots with the regression lines.

```{r plotA, fig.height=4, fig.width=6}
par(mar = c(4, 4, 2, 1))
plot(as.formula(paste('mpg ~', params$xvarA)), data = mtcars, 
     pch = 19, col = 'gray', main = paste("mpg vs", params$xvarA))
abline(lm(as.formula(paste('mpg ~', params$xvarA)), data = mtcars), col = 'red', lwd = 2)
```

```{r plotB, fig.height=4, fig.width=6}
par(mar = c(4, 4, 2, 1))
plot(as.formula(paste('mpg ~', params$xvarB)), data = mtcars, 
     pch = 19, col = 'gray', main = paste("mpg vs", params$xvarB))
abline(lm(as.formula(paste('mpg ~', params$xvarB)), data = mtcars), col = 'red', lwd = 2)
```

## Survival Analysis

```{r setup-survival, message=FALSE, warning=FALSE}
library(survival)
library(survminer)

# Default values if params are not provided
surv_dataset <- if(exists("params") && !is.null(params$surv_dataset)) params$surv_dataset else "lung"
surv_group <- if(exists("params") && !is.null(params$surv_group)) params$surv_group else "sex"

# Get the appropriate dataset
if(surv_dataset == "lung") {
  data <- survival::lung
  data$sex <- factor(data$sex, levels = c(1, 2), labels = c("Male", "Female"))
  time_var <- "time"
  status_var <- "status"
  title <- "Lung Cancer Dataset"
} else if(surv_dataset == "colon") {
  data <- survival::colon
  time_var <- "time"
  status_var <- "status"
  title <- "Colon Cancer Dataset"
} else if(surv_dataset == "veteran") {
  data <- survival::veteran
  time_var <- "time"
  status_var <- "status"
  title <- "Veteran's Administration Lung Cancer Dataset"
}
```

### Survival Analysis for `r title`

The survival analysis below shows the Kaplan-Meier curves for the `r surv_dataset` dataset, grouped by `r surv_group`.

```{r survival-plot, fig.height=6, fig.width=8}
# Create the survival formula
formula <- as.formula(paste("Surv(", time_var, ",", status_var, ") ~", surv_group))
...
ggsurvplot(fit, 
           ...
           ggtheme = theme_bw(),
           palette = "jco",
           title = paste("Survival Analysis by", surv_group))
```

## Sankey Diagram

```{r setup-sankey, message=FALSE, warning=FALSE}
library(networkD3)

# Default value if param is not provided
sankey_dataset <- if(exists("params") && !is.null(params$sankey_dataset)) params$sankey_dataset else "Energy Flow"

# Create the appropriate Sankey data
if(sankey_dataset == "Energy Flow") {
  nodes <- data.frame(name = c("Production", "Imports", "Exports", 
                               "Residential", "Commercial", "Industrial", 
                               "Transportation", "Electricity Generation"))
  
  links <- data.frame(
    source = c(0, 0, 0, 1, 2, 3, 4, 5, 6, 7),
    target = c(3, 4, 5, 7, 7, 3, 7, 6, 3, 4),
    value = c(10, 20, 30, 15, 5, 8, 12, 18, 7, 9)
  )
  
  title <- "Energy Flow Diagram"
} else {
  nodes <- data.frame(name = c("North America", "South America", "Europe", 
                               "Africa", "Asia", "Oceania"))
  
  links <- data.frame(
    source = c(0, 0, 1, 1, 2, 2, 3, 3, 4, 4),
    target = c(2, 4, 0, 2, 0, 4, 2, 4, 0, 2),
    value = c(15, 25, 10, 8, 20, 12, 18, 22, 30, 14)
  )
...
The Sankey diagram below visualizes the flow between different nodes in the `r sankey_dataset` dataset.

```{r sankey-plot, fig.height=6, fig.width=8}
...
Below is a sample of the data used in the survival analysis:
...
- `author`: Here you can list various author's names including their affiliation(s) and the email of the corresponding author.
- `date`: A date
- `abstract`: The abstract will be shown right after the title in smaller font size. 
- `logo1` and `logo2`: path of the logos that will be displayed on the bottom left and right of title page (after the front cover).
...
- `format - pdf`: In this template many of the Quarto options for Word output are listed in the YAML header. If you want to know more about these settings I recommend the [PDF format reference](https://quarto.org/docs/reference/formats/pdf.html) for a complete list of available options. For instance, you can adjust the figure and table references with `fig-title`, `tbl-title`, `fig-prefix`, and `tbl-prefix`.
    - `template-partials`: these 3 files are required for the layout of the front cover and title page.
    - `include-in-header`: The file 'styles/in-header.tex' determines the overall layout of the document.
    - Custom options:
      - `cover-bg-image`: path of the image on the cover page.
      - `cover-page-color`: background color of the cover page (provided as hex color code).
      - `cover-text-color`: color of the header on the cover page (provided as hex color code).
      - `cover-fade-effect`: if 'true' (default), the background color will overlay the image and fade away from top to bottom. If 'false' the image will overlay the background color.
