...
cat("plotA_x:", params$plotA_x, "\n")
fml <- as.formula(paste("mpg ~", params$plotA_x))
...
cat("plotB_x:", params$plotB_x, "\n")
fml <- as.formula(paste("mpg ~", params$plotB_x))
fit <- lm(fml, data = mtcars)
b <- coef(fit)
summary(fit)
```

The fitting result is $mpg = `r b[1]` + `r b[2]``r params$plotB_x`$.
