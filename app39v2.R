...
PLOT_CONFIG <- list(
  ...
  plot_C = list(
    label = "Treatment Selection (C)",
    max_plots = 6,
    height = "655px",
    has_annotations = TRUE,
    num_annotations = 7,  # Same as Plot A initially
    annotation_configs = list(
      list(suffix = "1", label = "Reference Label (for referencing figures):", placeholder = "Must start with 'fig-' , ie fig-C-1, etc"),
      list(suffix = "2", label = "Title:", placeholder = "My Title"),
      list(suffix = "3", label = "Subtitle:", placeholder = "My Subtitle"),
      list(suffix = "4", label = "Panel 1 Caption:", placeholder = "My Panel Caption"),
      list(suffix = "5", label = "Panel 2 Caption:", placeholder = "My Panel Caption"),
  ...
  plot_D = list(
    label = "Treatment Selection (D)",
    max_plots = 6,
    height = "655px",
    has_annotations = TRUE,
    num_annotations = 7,  # Same as Plot A initially
    annotation_configs = list(
      list(suffix = "1", label = "Reference Label (for referencing figures):", placeholder = "Must start with 'fig-' , ie fig-D-1, etc"),
      list(suffix = "2", label = "Title:", placeholder = "My Title"),
      list(suffix = "3", label = "Subtitle:", placeholder = "My Subtitle"),
      list(suffix = "4", label = "Panel 1 Caption:", placeholder = "My Panel Caption"),
      list(suffix = "5", label = "Panel 2 Caption:", placeholder = "My Panel Caption"),
...
# Generate Plot C content with configuration (mirrors Plot A structure)
generate_plot_C_content <- function(i, config, input) {
  ...
  plots <- create_plot_C_components(dat, i, values, input)
  table_component <- create_table_component_C(dat, i, input)
  
  # Combine with patchwork
  combine_plot_C_components(plots, table_component, values$figOvCap,
    values$title, values$subtitle)
}

# Generate Plot D content with configuration (mirrors Plot A structure)
generate_plot_D_content <- function(i, config, input) {
  ...
  plots <- create_plot_D_components(dat, i, values, input)
  table_component <- create_table_component_D(dat, i, input)
  
  # Combine with patchwork
  combine_plot_D_components(plots, table_component, values$figOvCap,
    values$title, values$subtitle)
}
...
# Create plot components for Plot C (initially mirrors Plot A)
create_plot_C_components <- function(dat, i, values, input) {
  base_theme <- create_base_plot_theme()
  values <- get_plot_values("plot_C", i, input)
  ...
  p1 <- ggplot(dat, aes(x = mpg)) +
    ...
    base_theme +
    labs(
      title = paste0("Distribution of MPG - Plot C (", i, ")"), 
      x = "MPG", 
      y = "Frequency",
      caption = values$plot1Cap 
    ) +
  ...
  p2 <- ggplot(dat, aes(x = hp)) +
    ...
    base_theme +
    labs(
      title = paste0("Distribution of HP - Plot C (", i, ")"), 
      x = "HP", 
      y = "Frequency",
      caption = values$plot2Cap 
    ) +
...
# Create plot components for Plot D (initially mirrors Plot A)
create_plot_D_components <- function(dat, i, values, input) {
  base_theme <- create_base_plot_theme()
  values <- get_plot_values("plot_D", i, input)
  ...
  p1 <- ggplot(dat, aes(x = mpg)) +
    ...
    base_theme +
    labs(
      title = paste0("Distribution of MPG - Plot D (", i, ")"), 
      x = "MPG", 
      y = "Frequency",
      caption = values$plot1Cap 
    ) +
  ...
  p2 <- ggplot(dat, aes(x = hp)) +
    ...
    base_theme +
    labs(
      title = paste0("Distribution of HP - Plot D (", i, ")"), 
      x = "HP", 
      y = "Frequency",
      caption = values$plot2Cap 
    ) +
...
# Create table component for Plot C
create_table_component_C <- function(dat, i, input) {
  # Get current values using helper function
  values <- get_plot_values("plot_C", i, input)
  table_data <- dat %>%
    select(mpg, cyl, hp) %>%
  ...
  gt_tab <- create_styled_gt_table_C(table_data, i)
...
# Create table component for Plot D
create_table_component_D <- function(dat, i, input) {
  # Get current values using helper function
  values <- get_plot_values("plot_D", i, input)
  table_data <- dat %>%
    select(mpg, cyl, hp) %>%
  ...
  gt_tab <- create_styled_gt_table_D(table_data, i)
...
# Create styled GT table for Plot C
create_styled_gt_table_C <- function(table_data, i) {
  table_data %>%
    gt() %>%
    tab_header(title = paste0("Sample Data - Plot C (", i, ")")) %>%
    tab_options(
      table.width = pct(100),
      table.layout = "fixed",
      data_row.padding = px(8),        
...
# Create styled GT table for Plot D
create_styled_gt_table_D <- function(table_data, i) {
  table_data %>%
    gt() %>%
    tab_header(title = paste0("Sample Data - Plot D (", i, ")")) %>%
    tab_options(
      table.width = pct(100),
      table.layout = "fixed",
      data_row.padding = px(8),        
...
# Combine plot components for Plot C
combine_plot_C_components <- function(plots, table_component, overall_caption,
    title, subtitle) {
...
# Combine plot components for Plot D
combine_plot_D_components <- function(plots, table_component, overall_caption,
    title, subtitle) {
  theme_border <- create_border_theme_D()
